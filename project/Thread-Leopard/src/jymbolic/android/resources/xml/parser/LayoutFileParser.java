...
import jymbolic.android.resources.controls.AndroidLayoutControl;
import jymbolic.android.resources.controls.LayoutControlFactory;
...
import jymbolic.android.resources.xml.parser.ARSCFileParser.AbstractResource;
import jymbolic.android.resources.xml.parser.ARSCFileParser.StringResource;
...
public class LayoutFileParser extends AbstractResourceParser {
	...
	public static void execute(String apkPath) {
...
//				logger.warn("Fragment without class name or id detected");
				...
				if (attr.getType() != NodeVisitor.TYPE_STRING) {
	...
	private void parseIncludeAttributes(String layoutFile, AXmlNode rootNode) {
		for (Entry<String, AXmlAttribute<?>> entry : rootNode.getAttributes().entrySet()) {
			...
			if (attrName.equals("layout")) {
				if ((attr.getType() == NodeVisitor.TYPE_REFERENCE || attr.getType() == NodeVisitor.TYPE_INT_HEX)
