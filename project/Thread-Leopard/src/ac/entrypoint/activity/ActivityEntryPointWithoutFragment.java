...
import ac.entrypoint.activity.AndroidCallBacks.ActivityInfo;
import ac.util.Log;
import soot.Body;
import soot.Local;
...
public class ActivityEntryPointWithoutFragment {
	...
	private DefaultLocalGenerator mLocalGenerator;
...
//	private Value intCounter;
	...
	public ActivityEntryPointWithoutFragment(SootClass activity) {
		...
		for(SootMethod sootMethod :callbacks ) {
			if(sootMethod.getParameterCount()>0) {
				for(int i=0; i< sootMethod.getParameterCount();i++) {
					...
					String key = sootMethod.getSignature()+sootMethod.getParameterType(i)+i;
...
//					body.getUnits().add(Jimple.v().newAssignStmt(val, NullConstant.v()));
	...
	private void addLifeCycleMethod(String methodName,SootClass myClass ,SootClass libClass) {
		...
		try {
			...
			Log.i("we cannot find lifecycle method ", methodName, " in class ", libClass.getName(),
